...
<body>
	...
	<div id="main">
		...
		<div class="outer1">
			<p style="font-size:25px;"><b>기안완료함</b> in ${loginUser.getDeptName() }</p>
			<br><br>
			<div class="list-1">
					...
					<input type="hidden" name="listType" id="listType" value="ds">
						...
						<c:choose>
							<c:when test="${tpi == null }">					
								<c:choose>
									...
									<c:otherwise>
										<li class=""><a class="page-link" href="dSendList.ap?cpage=${pi.currentPage -1 }">&lt;</a>  </li> 
									</c:otherwise>
								</c:choose>
								
								<c:forEach var="p" begin="${pi.startPage }" end="${pi.endPage }">
									<li class=""><a class="page-link" href="dSendList.ap?cpage=${p }">${p }</a></li>
								</c:forEach>
				...
				$(function(){
					...
					if(${not empty list}){
						$("table tbody").on("click", "tr", function(){
							location.href = 'detailSPrg.ap?no=' + $(this).children().eq(0).find("#num").val()+"&form="+ $(this).children().eq(3).text()+"&st=부서기안완료"; 
						}); 
					} 
				});
								...
								if(result.sList.length == 0){
									...
									for(var i = 0; i < result.sList.length; i++){
										...
										val1 += "<tr>"
											  ...
											  + "<td>" + result.sList[i].updateDate + "</td>"
											  ...
											  val1 += "<td>" + result.sList[i].appChange +"</td></tr>";
