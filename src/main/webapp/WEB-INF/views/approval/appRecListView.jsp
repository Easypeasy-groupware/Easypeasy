...
<body>
	...
	<div id="main">
		...
		<div class="outer1">
			<p style="font-size:25px;"><b>결재문서함</b></p>
			<br>
			...
			<div class="list1-1">
				<a href="statusList.ap?tstatus=전체&listType=c">전체</a> | <a href="statusList.ap?tstatus=진행중&listType=c">진행</a> | <a href="statusList.ap?tstatus=결재&listType=c">완료</a> | <a href="statusList.ap?tstatus=반려&listType=c">반려</a>
			</div>
						...
						<c:choose>
							<c:when test="${tpi == null }">
								<c:choose>
									...
									<c:otherwise>
										<li class=""><a class="page-link" href="recList.ap?cpage=${pi.currentPage -1 }">&lt;</a>  </li> 
									</c:otherwise>
								</c:choose>
								
								<c:forEach var="p" begin="${pi.startPage }" end="${pi.endPage }">
									<li class=""><a class="page-link" href="recList.ap?cpage=${p }">${p }</a></li>
								</c:forEach>
								...
								<c:choose>
									...
									<c:otherwise>
										<li class="page-item"><a class="page-link" href="recList.ap?cpage=${pi.currentPage + 1 }">&gt;</a></li>
									</c:otherwise>
								</c:choose>
							</c:when>
							<c:otherwise>
				...
				$(function(){
					...
					if(${not empty list}){
						$("table tbody").on("click", "tr", function(){
							location.href = 'detailRec.ap?no=' + $(this).children().eq(0).find("#num").val()+"&form="+ $(this).children().eq(3).text()+"&st=전체"; 
						}); 
					}
				});
